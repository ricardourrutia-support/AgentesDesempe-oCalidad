...
def process_ventas(df, d_from, d_to):
    ...
    if "createdAt_local" in df.columns:
        df["fecha"] = df["createdAt_local"].apply(to_date)
    ...
    if "ds_agent_email" in df.columns:
...
def process_performance(df, d_from, d_to):
    ...
    out = df.groupby(["agente", "fecha"], as_index=False).agg(agg_dict)
...
AGENT_COLS = [
    "RUT", "Nombre", "Primer Apellido", "Segundo Apellido", 
    "Email De Contacto", "Cargo", "Contrato", "Fecha Ingreso", "Supervisor"
]
...
def build_daily(df_list, agentes_df):
    ...
    float_cols = ["NPS", "CSAT", "FIRT", "%FIRT", "FURT", "%FURT", "Nota_Auditorias"]
    for c in float_cols:
        if c in merged.columns:
...
def build_weekly(df_daily):
    ...
    agg_dict = {
        "Q_Encuestas":"sum", "NPS":"mean", "CSAT":"mean", "FIRT":"mean", 
        "%FIRT":"mean", "FURT":"mean", "%FURT":"mean", "Q_Reopen":"sum", 
        "Q_Tickets":"sum", "Q_Tickets_Resueltos":"sum", "Q_Auditorias":"sum", 
        "Nota_Auditorias":"mean", "Ventas_Totales":"sum", "Ventas_Compartidas":"sum", 
    ...
    cols_for_merge = [c for c in AGENT_COLS if c in df_daily.columns]
    weekly = weekly.merge(df_daily[cols_for_merge].drop_duplicates(), on="Email De Contacto", how="left")
...
def build_summary(df_daily):
    ...
    cols_to_agg = {k: v for k, v in agg_dict.items() if k in df_daily.columns}
    ...
    cols_for_merge = [c for c in AGENT_COLS if c in df_daily.columns]
    resumen = resumen.merge(df_daily[cols_for_merge].drop_duplicates(), on="Email De Contacto", how="left")
...
def build_coordinadores(df_daily):
    if df_daily.empty or "Supervisor" not in df_daily.columns:
        return pd.DataFrame()
    ...
    for c in ["NPS", "CSAT", "FIRT", "%FIRT", "FURT", "%FURT", "Nota_Auditorias"]:
        if c in resumen_coord.columns:
            resumen_coord[c] = resumen_coord[c].round(2)
